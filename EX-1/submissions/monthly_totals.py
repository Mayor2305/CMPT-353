...
def pivot_months_pandas(data):
    ...
    # 2.)
    new_data = data.groupby(['name', 'month'])
    
    # 3 and 4
    sum = new_data.aggregate('sum').reset_index()
    counts = new_data.aggregate('count').reset_index() 
    
    monthly = sum.pivot(index = 'name', columns = 'month')['precipitation']
    counts = counts.pivot(index = 'name', columns = 'month')['precipitation']
